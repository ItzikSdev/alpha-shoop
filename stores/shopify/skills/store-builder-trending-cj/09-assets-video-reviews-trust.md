...
### 7.A.1 — Choosing the hero image (v2.1, Itzik's rule)

The hero is the first gallery image — the photo on the homepage card, the
first thing a shopper sees on the product page, and the image in link
previews. It is chosen by this procedure, never by "whatever CJ listed
first." Three requirements, in priority order:

1. **Not pixelated.** Hard gate.
2. **No Chinese text anywhere in the image** — packaging, stickers,
   watermarks, overlays. Hard gate.
3. **Prefer a photo of someone using the product** (a parent wearing
   the carrier, a baby playing with the toy) over a bare product shot.
   Tie-breaker among images that pass 1 and 2.

**Candidates:** every image in the product's gallery, plus every photo
from its imported reviews (real buyers using the real product are often
the best "in use" shots — re-hosted per 7.D #35 first).

**Tooling — [markitdown](https://github.com/microsoft/markitdown) with
OCR.** For an image, markitdown returns its EXIF metadata plus, when
given an LLM client, a description generated by a vision model — and the
prompt is ours to write, so that one call does both the OCR (transcribe
every visible character) and the "is a person using it" judgement:
...
**The three checks — measured, not eyeballed:**

| Check | How | Pass |
|---|---|---|
| 1. Not pixelated | Pillow: real pixel size of the ORIGINAL file (not a thumbnail). OpenCV: variance of the Laplacian on the grayscale image resized to 1024px on the long side. | Short side **≥ 1000px** and Laplacian variance **≥ 100**. Also reject images whose file is small for their dimensions (an upscaled thumbnail: < 0.5 bytes/pixel as JPEG). |
| 2. No Chinese text | Run the CJK regex over markitdown's `ocr_text`: `[\u3400-\u4dbf\u4e00-\u9fff\uf900-\ufaff\U00020000-\U0002a6df]` | **Zero** matches. Any Chinese character, however small, fails. |
| 3. Person using it | `person_using_product` from the same call | `true` wins among images that pass 1 and 2. |

**Pick:** filter to images passing checks 1 and 2 → among those, prefer
`person_using_product: true` → tie-break by higher Laplacian variance,
then larger short side. If **no** image passes checks 1 and 2, the
product has no usable hero — report it; don't lower the bar and don't
crop or blur the text out of a failing image and call it clean.

**Apply it:** make the chosen image position 1 in the product's media
in Shopify (Admin or `productReorderMedia`), and save the decision to
`store-profiles/alphaforbaby/hero-selection/<handle>.json`:

```json
{
  "handle": "ergonomic-baby-hip-carrier",
  "chosen": {"url": "https://cdn.shopify.com/...", "short_side": 1600,
             "laplacian_var": 312.4, "cjk_chars": 0,
             "person_using_product": true},
  "rejected": [{"url": "...", "reason": "cjk_text: 3 chars on the box"}],
  "model": "<vision model used>", "checked_at": "2026-09-21"
}
```

Level 14's `TestHeroImage` reads this file and checks the rendered page
against it.
...
## 7.F — Asset status at v2.2 (2026-09-21), measured

- **Review photos: 297 of 297 across the 9 products are on
  `cdn.shopify.com`, and all 297 return HTTP 200.** 7.D #35's hotlink
  problem is gone from the metafield data.
- **The last supplier-host leak was not an image.** `aliyuncs` still
  appeared twice in the carrier's served HTML after every photo was
  re-hosted. It was not in any `<img>` (67 img tags, 0 supplier-hosted) —
  the route queried the AG Product Reviews blob
  (`air_reviews_product.data`), consumed only its approved-count summary,
  and serialized the entire blob, supplier CDN hostnames and all, into the
  page's loader payload. Fixed by destructuring it out of the product
  before it is returned. **When checking 7.D #6, grep the whole served
  HTML for the supplier host, not just `img[src]`** — a field you query
  but barely use still ships to the browser in full.
- **Product-demo video (Slot 2): all 9 products have one**, a real mp4
  served from Shopify's CDN, in `media`. The "no video on 6 products" gap
  is closed.
- **Hero-image procedure (7.A.1 / Level 01 rule 8): NOT done.** No
  `store-profiles/alphaforbaby/hero-selection` directory exists, so no
  product has a recorded hero decision and nothing has been run through
  the markitdown + OCR checks. This is unstarted work, not a partial pass.

### 7.A.1 — two corrections from the first real run (v2.3)

**The `< 0.5 bytes/pixel` upscaled-thumbnail check is mis-calibrated and
must not be applied literally.** Measured across all 271 real candidate
images on the three live products: range **0.0256 – 1.3789 b/px, median
0.1787**, and only **7 of 271** images reach 0.5. That density is far
above what a normal JPEG produces, so at 0.5 the rule rejects every
gallery image and every review photo, and the procedure concludes that
no product has a usable hero — which is a measurement artefact, not a
finding about the images. Use **0.02 b/px**, which still catches a
genuine upscaled thumbnail (the worst real image measured 0.0256). The
two hard gates — short side ≥ 1000px and Laplacian variance ≥ 100 —
are correct as written and were left alone.

**One low-resolution vision pass is not enough for a hard gate.** The
single `markitdown` call per image (which downscales before sending)
returned `cjk_chars: 0` and `person_using_product: true` for a carrier
photo that actually contained **18 Chinese characters** on an
instruction leaflet and showed **nobody using the product** — the only
people in the frame were printed on that leaflet. It would have been
chosen as the hero, because its clutter also gave it the highest
Laplacian variance in the set. Two changes, both of which RAISE the
bar:

1. **Tile the OCR at native resolution.** Split the image into a 2x3
   grid, send each tile separately, and sum the CJK hits. Small print
   survives this; it does not survive a single downscaled pass.
2. **Ask the "person" question explicitly against the packaging case.**
   The prompt must say that `person_using_product` is FALSE when the
   only people are printed on packaging, a manual, a leaflet, a box or
   a poster inside the photo. Without that sentence the model counts a
   photo OF a photo.

Sequence that worked, and is cheap: run the mechanical gate 1 locally
on every candidate first (it is free), then the vision pass on the
survivors, then the tiled re-check on the ~6 finalists. On these three
products that was 271 → 35 → 18 images actually sent for the strict
pass.

**Expect the hero to come from the reviews, not the gallery.** All
three chosen heroes were buyer review photos (1500x2000), because CJ's
own gallery images are mostly 800x800 and fail the short-side gate
outright — the Domino had **no** gallery image that passed. Review
photos need to be added to the product's media (`productCreateMedia`)
before `productReorderMedia` can put them first; Shopify appends a
uniqueness suffix to the filename when it copies the file in, so
verify position 1 by filename **stem**, not by full URL.
