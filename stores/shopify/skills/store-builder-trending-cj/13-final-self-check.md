...
- [ ] **HERO IMAGE (v2.1, Level 01 rule 8)**: each live product's first image was chosen by the Level 09 7.A.1 markitdown + OCR procedure and passes all three rules — short side ≥ 1000px and Laplacian variance ≥ 100 (not pixelated), zero Chinese characters in the OCR text, and a photo of someone using the product whenever a clean one exists; the decision is saved in `store-profiles/alphaforbaby/hero-selection/<handle>.json`
