# TIMEFOR BABY — Hydrogen storefront

A **Shopify Hydrogen** (React / React-Router 7, deployed on **Oxygen**) headless storefront
for the TIMEFOR BABY store (`kgg8n0-k0.myshopify.com` → timeforbaby.alpha-tech.live).
...
(`../timeforbaby.alpha-tech.live/style/site.json` + `product.json`). Font sizes use a normal
...
## Local development
```bash
cd stores/shopify/hydrogen-timeforbaby
