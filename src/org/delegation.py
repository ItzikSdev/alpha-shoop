...
def load_store_design() -> tuple[str, str]:
    ...
    for s in list_stores():
        ...
        # tolerant to suffixes (e.g. store "alphaforbaby_kgg" → folder "alphaforbaby").
...
is `product.json` (in stores/shopify/alphaforbaby.alpha-tech.live/style/). Grace changes the design by
...
async def _linus_pick_task(linus: Agent | None, grace: Agent, company: Company) -> tuple[str, str]:
    ...
    try:
        ...
        owner = read_store_docs("alphaforbaby").get("owner", "")
        if owner:
