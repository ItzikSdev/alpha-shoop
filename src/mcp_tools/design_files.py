...
def _store_dir(store_slug: str) -> Path:
    """stores/shopify/<slug>/, tolerant to naming (e.g. slug 'alphaforbaby' matches a
    folder 'alphaforbaby.alpha-tech.live'). Falls back to the only store folder when
    ...
    if len(dirs) == 1:
        return dirs[0]
    return base / (store_slug or "alphaforbaby")


def read_store_docs(store_slug: str = "alphaforbaby", changelog_chars: int = 1600) -> dict:
